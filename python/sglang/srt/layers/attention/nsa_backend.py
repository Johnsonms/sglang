        ...
        if _USE_FUSED_METADATA_COPY:
            try:
                ...
                if _VERIFY_FUSED_METADATA_COPY:
                    print(
                        f"[VERIFY] Running fused metadata copy verification (bs={bs}, mode={forward_mode})..."
                    )
                    print("********Verifying fused kernel results...")
                    ...
                    ref_nsa_cache_seqlens = torch.zeros_like(
                        metadata.nsa_cache_seqlens_int32
                    )
                    ref_nsa_seqlens_expanded = torch.zeros_like(
                        metadata.nsa_seqlens_expanded
                    )
                    ...
                    if forward_mode.is_decode_or_idle():
                        # Decode mode
                        ref_page_table_1[:, : precomputed.max_len].copy_(
                            precomputed.page_indices
                        )
                        ref_nsa_cache_seqlens.copy_(precomputed.nsa_cache_seqlens)
                    elif forward_mode.is_target_verify():
                        # Target verify mode
                        ref_page_table_1[:, : precomputed.max_seqlen_k].copy_(
                            precomputed.page_indices
                        )
                        ...
                        ref_nsa_seqlens_expanded[:size].copy_(
                            precomputed.seqlens_expanded
                        )
                        ref_nsa_cache_seqlens[:size].copy_(
                            precomputed.nsa_cache_seqlens
                        )
                    ...
                    if precomputed.real_page_table is not None:
                        ...
                        ref_real_page_table[:rows, :cols].copy_(
                            precomputed.real_page_table
                        )
                    ...
                    check_tensor_equal(
                        "cache_seqlens", fused_cache_seqlens, ref_cache_seqlens
                    )
                    check_tensor_equal(
                        "cu_seqlens_k", fused_cu_seqlens_k, ref_cu_seqlens_k
                    )
                    ...
                    print(
                        f"[VERIFY] ✓ Verification passed for bs={bs}, mode={forward_mode}"
                    )
                else:
                    print(
                        f"[VERIFY] ✗ Verification failed for bs={bs}, mode={forward_mode}"
                    )
            except ImportError:
                print(
        ...
        if envs.SGLANG_NSA_ENABLE_MTP_PRECOMPUTE_METADATA.get():
            ...
            if self.speculative_num_steps >= 3:
                try:
                    ...
                    # Verification: compare fused kernel results against individual copies
                    if _VERIFY_FUSED_METADATA_COPY:
                        # print(f"[VERIFY] Running multi-backend fused metadata copy verification (bs={bs}, num_backends=3)...")
                        ...
                        for idx, metadata in enumerate(
                            ...
                            fused_page_table_1 = metadata.page_table_1.clone()
                            fused_nsa_cache_seqlens = (
                                metadata.nsa_cache_seqlens_int32.clone()
                            )
                            fused_nsa_cu_seqlens_k = metadata.nsa_cu_seqlens_k.clone()
                            fused_real_page_table = (
                                metadata.real_page_table.clone()
                            ...
                            fused_flashmla_metadata = None
                            if precomputed.flashmla_metadata is not None:
                                fused_flashmla_num_splits = (
                                    metadata.flashmla_metadata.num_splits.clone()
                                )
                                fused_flashmla_metadata = (
                        ...
                        for idx in range(3):
                            ...
                            # Create reference tensors (zeroed out)
                            ref_cache_seqlens = torch.zeros_like(
                                metadata.cache_seqlens_int32
                            )
                            ref_cu_seqlens_k = torch.zeros_like(metadata.cu_seqlens_k)
                            ref_page_table_1 = torch.zeros_like(metadata.page_table_1)
                            ...
                            ref_nsa_cu_seqlens_k = torch.zeros_like(
                                metadata.nsa_cu_seqlens_k
                            )
                            ref_real_page_table = (
                                torch.zeros_like(metadata.real_page_table)
                        ...
                        def check_tensor_equal(backend_idx, name, fused, ref):
                            if not torch.equal(fused, ref):
                                max_diff = (
                                    (fused.float() - ref.float()).abs().max().item()
                                )
                                mismatched_elements = (fused != ref).sum().item()
                                total_elements = fused.numel()
                                raise RuntimeError(
                        ...
                        # print(f"[VERIFY] ✓ Multi-backend verification passed for bs={bs}, num_backends=3")
