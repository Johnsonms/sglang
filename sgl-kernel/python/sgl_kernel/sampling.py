...
def _top_k_renorm_probs_compiled(
    probs: torch.Tensor,
    maybe_top_k_arr: Optional[torch.Tensor],
    top_k_val: int,
) -> torch.Tensor:
    """Compiled version using sgl_kernel torch ops."""
    probs = probs.float()
...
def top_k_renorm_probs_compiled(
    probs: torch.Tensor,
    top_k: Union[torch.Tensor, int],
) -> torch.Tensor:
    r"""Compiled version: Fused GPU kernel for renormalizing probabilities by top-k thresholding.

    Uses statically compiled FlashInfer kernels via sgl_kernel torch ops.
    ...
    """
    return _top_k_renorm_probs_compiled(probs, *_to_tensor_scalar_tuple(top_k))
...
def top_k_renorm_probs(
    ...
    r"""JIT version: Fused GPU kernel for renormalizing probabilities by top-k thresholding.

    Uses FlashInfer JIT-compiled kernels.
...
def _top_p_renorm_probs_compiled(
    probs: torch.Tensor,
...
def top_p_renorm_probs_compiled(
    probs: torch.Tensor,
    top_p: Union[torch.Tensor, float],
) -> torch.Tensor:
    r"""Compiled version: Fused GPU kernel for renormalizing probabilities by top-p thresholding.
    ...
    renorm_probs: torch.Tensor
        Renormalized probabilities, shape ``(batch_size, num_classes)``.
    """
    return _top_p_renorm_probs_compiled(probs, *_to_tensor_scalar_tuple(top_p))
...
def top_p_renorm_probs(
    ...
    r"""JIT version: Fused GPU kernel for renormalizing probabilities by top-p thresholding.
...
def _top_k_mask_logits_compiled(
    logits: torch.Tensor,
    maybe_top_k_arr: Optional[torch.Tensor],
    top_k_val: int,
) -> torch.Tensor:
...
def top_k_mask_logits_compiled(
    logits: torch.Tensor,
    top_k: Union[torch.Tensor, int],
) -> torch.Tensor:
    r"""Compiled version: Fused GPU kernel for masking logits by top-k thresholding.
    ...
    """
    return _top_k_mask_logits_compiled(logits, *_to_tensor_scalar_tuple(top_k))
...
def top_k_mask_logits(
    ...
    r"""JIT version: Fused GPU kernel for masking logits by top-k thresholding.
