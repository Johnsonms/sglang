...
def torch_gemma_rmsnorm(input, weight, eps=1e-6):
    ...
    variance = input.pow(2).mean(dim=-1, keepdim=True)
    output = input * torch.rsqrt(variance + eps)
    output = output * (weight + 1.0)  # Gemma adds 1 to weight
    return output
...
def torch_gemma_fused_add_rmsnorm(input, residual, weight, eps=1e-6):
    ...
    # Step 2: Gemma RMSNorm on residual
    variance = residual_updated.pow(2).mean(dim=-1, keepdim=True)
    output = residual_updated * torch.rsqrt(variance + eps)
    output = output * (weight + 1.0)  # Gemma adds 1 to weight
...
def calculate_diff_rmsnorm(batch_size, hidden_size):
    """Compare Torch reference, compiled, and JIT for rmsnorm correctness."""
    torch.manual_seed(42)
    device = torch.device("cuda")
    dtype = torch.float16  # Compiled kernels require float16
    ...
    compiled_output = sgl_kernel.rmsnorm_compiled(input, weight, eps)
    jit_output = sgl_kernel.rmsnorm(input, weight, eps)

    torch.testing.assert_close(torch_output, compiled_output, rtol=1e-2, atol=1e-2)
...
def calculate_diff_fused_add_rmsnorm(batch_size, hidden_size):
    """Compare Torch reference, compiled, and JIT for fused_add_rmsnorm correctness."""
    torch.manual_seed(42)
    device = torch.device("cuda")
    dtype = torch.float16  # Compiled kernels require float16
    ...
    residual_compiled = residual_torch.clone()
    residual_jit = residual_torch.clone()
    weight = torch.randn(hidden_size, device=device, dtype=dtype)
    eps = 1e-6

    torch_output, _ = torch_fused_add_rmsnorm(input.clone(), residual_torch, weight, eps)

    # Compiled version (in-place)
    input_compiled = input.clone()
    sgl_kernel.fused_add_rmsnorm_compiled(input_compiled, residual_compiled, weight, eps)

    # JIT version (in-place)
    input_jit = input.clone()
    sgl_kernel.fused_add_rmsnorm(input_jit, residual_jit, weight, eps)

    torch.testing.assert_close(torch_output, input_compiled, rtol=1e-2, atol=1e-2)
...
def calculate_diff_gemma_rmsnorm(batch_size, hidden_size):
    """Compare Torch reference, compiled, and JIT for gemma_rmsnorm correctness."""
    torch.manual_seed(42)
    device = torch.device("cuda")
    dtype = torch.float16  # Compiled kernels require float16
    ...
    compiled_output = sgl_kernel.gemma_rmsnorm_compiled(input, weight, eps)
...
def calculate_diff_gemma_fused_add_rmsnorm(batch_size, hidden_size):
    """Compare Torch reference, compiled, and JIT for gemma_fused_add_rmsnorm correctness."""
    torch.manual_seed(42)
    device = torch.device("cuda")
    dtype = torch.float16  # Compiled kernels require float16
    ...
    torch_output, _ = torch_gemma_fused_add_rmsnorm(input.clone(), residual_torch, weight, eps)

    # Compiled version (in-place)
    input_compiled = input.clone()
    sgl_kernel.gemma_fused_add_rmsnorm_compiled(input_compiled, residual_compiled, weight, eps)
...
def benchmark_rmsnorm(batch_size, hidden_size, provider):
    ...
    if provider == "torch":
        ...
        fn = lambda: sgl_kernel.rmsnorm_compiled(input.clone(), weight, eps)
...
def benchmark_fused_add_rmsnorm(batch_size, hidden_size, provider):
    ...
    if provider == "torch":
        fn = lambda: torch_fused_add_rmsnorm(input.clone(), residual.clone(), weight, eps)
    elif provider == "sgl_compiled":
        def compiled_fn():
            inp = input.clone()
            res = residual.clone()
            sgl_kernel.fused_add_rmsnorm_compiled(inp, res, weight, eps)
            return inp
        fn = compiled_fn
    elif provider == "sgl_jit":
        def jit_fn():
...
def benchmark_gemma_rmsnorm(batch_size, hidden_size, provider):
    ...
    if provider == "torch":
        ...
        fn = lambda: sgl_kernel.gemma_rmsnorm_compiled(input.clone(), weight, eps)
    elif provider == "sgl_jit":
...
def benchmark_gemma_fused_add_rmsnorm(batch_size, hidden_size, provider):
    ...
    if provider == "torch":
        fn = lambda: torch_gemma_fused_add_rmsnorm(input.clone(), residual.clone(), weight, eps)
    elif provider == "sgl_compiled":
        def compiled_fn():
            inp = input.clone()
            res = residual.clone()
            sgl_kernel.gemma_fused_add_rmsnorm_compiled(inp, res, weight, eps)
            return inp
        fn = compiled_fn
    elif provider == "sgl_jit":
        def jit_fn():
...
if __name__ == "__main__":
    ...
    if IS_CI:
        test_configs = [configs[0]] if configs else [(32, 4096)]
    else:
